...
### Gemma 4（`gemma-4-E4B-it-UD-Q8_K_XL.gguf`）啟動建議

Gemma 4 的 thinking 控制方式和 Qwen3.5 不同。依 Google / Unsloth 的 model card，`gemma-4-E4B-it` 建議採樣參數為：

- `temperature=1.0`
- `top_p=0.95`
- `top_k=64`

在新版 llama.cpp，Gemma 4 也建議直接用 `--reasoning off`，而不是再透過 `chat_template_kwargs.enable_thinking=false` 關閉 thinking。

```bash
LD_LIBRARY_PATH=~/dev/llama-bin ~/dev/llama-bin/llama-server \
    -m ~/dev/model/gemma-4-E4B-it-UD-Q8_K_XL.gguf \
    --port 8080 \
    --jinja \
    -c 2048 \
    --parallel 1 \
...
> （如 `Hy-MT2`、`Qwen3.5`），否則會使用預設名稱 `local-model`，無法觸發該模型家族的專用功能
> （例如 Hunyuan-MT 的專用 prompt 路徑、Qwen3.5-UD 的日文名字保護與專用 sampling 參數）。
...
> **Gemma 4 例外：** Gemma 4 不是 DeepSeek / Qwen 的思考格式。對 `gemma-4-E4B-it`，本專案目前改用 per-request `reasoning: off`，並搭配 `reasoning_format: none`，避免依賴新版 llama.cpp 已開始淡出的 `chat_template_kwargs.enable_thinking` 控制方式。
