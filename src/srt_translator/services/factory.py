...
class TranslationService:
    ...
    def _incr_stat(self, key: str, amount: int | float = 1) -> None:
        """安全地增加統計值"""
        ...
        if hasattr(client, "translate_batch"):
            ...
            for result in completed:
                if isinstance(result, Exception):
                    logger.error(f"批量翻譯中的項目發生錯誤: {result!s}")
                    continue
    ...
    async def translate_subtitle_file(
        ...
        try:
            ...
            logger.error(f"翻譯字幕檔案時發生錯誤: {e!s}")
    ...
    def _apply_translation(self, subtitle: Any, translation: str, display_mode: str) -> None:
        ...
        elif display_mode == "原文在上" or display_mode == "雙語對照":
...
class ModelService:
    ...
    def get_recommended_model(self, task_type: str = "translation", provider: str | None = None) -> str:
        """根據任務類型獲取推薦模型
    ...
    async def cleanup(self) -> None:
        ...
        for llm_type, client in self.translation_clients.items():
            try:
                ...
                logger.error(f"關閉 {llm_type} 翻譯客戶端時發生錯誤: {e!s}")
...
class TranslationTaskManager:
    ...
    def _complete_wrapper(
        self, file_path: str, original_callback: Callable[..., Any] | None
