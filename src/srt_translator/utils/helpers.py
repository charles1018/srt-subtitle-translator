...
def is_valid_subtitle_file(file_path: str) -> bool:
    ...
    try:
        ...
        logger.warning(f"檢查字幕文件 {file_path} 時發生錯誤: {str(e)}")
        return False
...
class LocaleManager:
    ...
    def _load_locale(self, locale_code: str) -> bool:
        ...
        try:
            ...
            logger.error(f"載入語言文件 {locale_file} 失敗: {str(e)}")
            # 創建空字典以避免重複載入嘗試
            self.translations[locale_code] = {}
            return False
    ...
    def get_text(self, key: str, **kwargs) -> str:
        ...
        if kwargs:
            try:
                ...
                logger.warning(f"格式化文本 '{key}' 時缺少參數: {str(e)}")
                return text
    ...
    def save_translations(self, locale_code: str = None) -> bool:
        ...
        try:
            ...
            logger.error(f"保存語言文件 {locale_file} 失敗: {str(e)}")
            return False
