...
class ConfigManager:
    ...
    def _load_specific_config(self, config_type: str) -> None:
        ...
        try:
            ...
            logger.error(f"載入配置失敗 {config_path}: {str(e)}")
            # 使用預設配置
            self.configs[config_type] = default_config
        ...
        try:
            ...
            for listener in self.listeners:
                try:
                    ...
                    logger.error(f"通知配置監聽器時發生錯誤: {str(e)}")
            
            return True
        except Exception as e:
            logger.error(f"儲存配置失敗 {config_path}: {str(e)}")
            return False
        ...
        try:
            ...
            for file in Path(backup_dir).glob(backup_pattern):
                try:
                    ...
                    logger.warning(f"讀取備份檔案 {file} 失敗: {str(e)}")
