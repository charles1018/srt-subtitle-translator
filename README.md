...
[![Tests](https://img.shields.io/badge/tests-941%20passed-brightgreen.svg)](tests/)
...
- 🆕 **結構-文本分離批次翻譯**：將字幕結構（timestamp/index）與文本分離，多句合併單一 API 請求翻譯後重組（1:1 行數驗證 + 自動退回逐句），大幅降低 token 消耗並消除 LLM 損壞結構的風險；CLI 用 `--structure-text`，GUI 勾選「批次翻譯」
...
- **非 GUI 基線**：941 passed / 0 skipped / 0 warnings
