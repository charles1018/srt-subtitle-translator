...
async def test_single_file_translation_basic(
    ...
    e2e_temp_dir: Path,
    mock_all_services_for_workflow,
    srt_comparator
):
    """測試 1：基本單檔案翻譯
...
async def test_cache_hit_scenario(
    ...
    # 執行翻譯並儲存快取
    translation1 = await translation_service.translate_text(
        test_text,
        context,
        "openai",
        model_name
...
@pytest.mark.asyncio
async def test_cache_for_multiple_texts(
    mock_all_services_for_workflow
):
    """測試 5：多個文本的快取
...
@pytest.mark.asyncio
async def test_english_to_chinese_translation(
    sample_srt_path: Path,
    mock_all_services_for_workflow
):
    """測試 6：英文 -> 繁體中文翻譯
...
@pytest.mark.asyncio
async def test_japanese_to_chinese_translation(
